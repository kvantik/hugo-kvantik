...
def deadline(tour_num):
    return ['', 'октября', 'ноября', 'декабря', 'января', 'февраля', 'марта', 'апреля', 'мая', 'июня', 'июля', 'августа', 'сентября'][tour_num]
...
def text_finalize(text):
  ...
  text = text.replace('  ',' ') # двойные пробелы
  text = text.replace(' – ',' — ') # дефис на длинное тире
  text = text.replace('Художник Николай Крутиков','') # долой подписи
  text = text.replace(' × ','×') # 
  return text
