...
def get_directory(filename, type):
...
def get_authors(lines, type="math"):
    if type!="math":
        return dict()
...
def parse_tour(lines, type="math"):
...
def extract(filename, type='math'):
    directory = get_directory(filename, type)
    pages = get_pages(type)    
  ...
  - author:
    ...
    with open(directory+str(tour)+'.yaml','w') as text:        
        text.writelines(yaml.dump(parse_tour(lines), allow_unicode=True, default_flow_style=False, width = 10000))
